...
public class DownloadJob implements Runnable{
    ...
    private String job() throws IOException, InterruptedException {
        folderName = c.getDownload_Folder() + user.getId();
        // Create the process
        user.setDownloadFormat(DownloadFormat.FLAC);
        String[] commands = 
        new String[]{c.getPython_executable(), "-m", "deemix","-b", "flac","-p",folderName, "url", link, "arl",c.getArl()} ;
