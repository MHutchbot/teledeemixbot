...
public class CommandHandler {
    ...
    public boolean handle(String text,long chat_id,String userId){
        ...
        switch(EmojiParser.parseToAliases(text)){
            case "/start":
                Bot.getInstance().sendKeyboard("Hi, welcome to Losthopebot :musical_note:, i'm here to spread the music all over the world :earth_africa:",chat_id);
                Bot.getInstance().sendMessage("To start, just type a spotify or deezer link",chat_id);
                return true;
            case "/bug":
                Bot.getInstance().sendKeyboard("LostHopeBot is still in beta phase, so it's normal that there are some inconveniences, such as unsent music or other problems. \n\nIf you want to report a bug do so via Github issues at https://github.com/replydev/DeemixBot/issues, taking care to specify the type of bug and how to replicate it. \n\nI will do my best to fix it as soon as possible. Thanks for your help!",chat_id);
                return true;
            case ":cd: Track mode":
